...
COLUMNS = [
    ...
    {"id": "assignee", "title": "Assignee", "width": 100},
]
...
app.layout = html.Div(
    ...
    children=[
        html.Div(
            [
                html.H1("Range Cell Example", id="title"),
                html.P(
                    "Click on a progress cell to edit. Rows 1 and 5 are readonly.",
                    id="subtitle",
                ),
                html.Button(
                    "Toggle Dark Mode",
...
def show_data(grid_data, theme):
    ...
    for row in grid_data:
        progress = row.get("progress", {}).get("value", 0)
        bar = "#" * (progress // 10) + "-" * (10 - progress // 10)
        lines.append(f"{row['task']}: [{bar}] {progress}% ({row['assignee']})")
